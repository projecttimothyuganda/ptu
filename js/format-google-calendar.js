...
window.formatGoogleCalendar = function () {
    ...
    var renderList = function renderList(data, settings) {
        ...
        //James Crooke additions start
        $(document).ready(function() {
         
        //countdown variables
        var str = $( "span.date" ).first().text();
        var splitText = str.split(" ");
        var dateWordNumber = splitText.length;
        var countdownMonth = splitText[1];
        var threeLetterMonth = countdownMonth.slice(0, 3);
        var countdownDate = splitText[2];
        var countdownYear = splitText[3];
        var countdownTime = splitText[5];
        ...
        //possible error handler	
     	/*try {
        	...
        	$('#notice-board').delay('1100').slideDown(500);
        	}
        	
        //time array
        if (countdownTime.length === 7) {
        var countdownTimeSplit = countdownTime.split("", 5);
        var countdownHours = countdownTime.split("", 2).join(); // double digit number
        var countdownMin = countdownTime.substr(3, 2);
        } else if (countdownTime.length === 6) {
        var countdownTimeSplit = countdownTime.split("", 4);
        var countdownHours = countdownTime.split("", 1); // single digit number
        var countdownMin = countdownTime.substr(2, 2);
        } 
        
        console.log(dateWordNumber);
        
        var amPm = countdownTime.slice(-2);
        
        if(amPm == "PM" && countdownHours<12) countdownHours = ~~countdownHours + 12;
		if(amPm == "AM" && countdownHours==12) countdownHours = ~~countdownHours - 12;
        
        var countdownTimeRejoin = countdownTimeSplit.join("");
		var countdownFormattedTime = countdownMonth + " " + countdownDate + " " + countdownYear + " " + countdownHours + ":" + countdownMin + ":00"; 
		var formattedTimeString = countdownFormattedTime.toString();
		
		var countDownDate = new Date(formattedTimeString).getTime();
 ...
 //country identifier
  ...
  // error handler
  		...
  		document.getElementById("countdown").innerHTML ="Countdown to next event: " + days + "d " + hours + "h "
  + minutes + "m " + seconds + "s. ";
  // Display the result in the element with id="countdown" Uganda
  ...
  // If the count down is finished, write some text 
  		if (distance < 0) {
    		clearInterval(x);
    		document.getElementById("countdown").innerHTML ="A Project Timothy event is happening right now.";
  		}
		}, 1000);	
        } 
    ...
    var getFormattedTime = function getFormattedTime(date) {
	 ...
	 //James Crooke Additions to display time in local time begin    
       ...
       // var posOrNeg = Math.sign(timeDifferenceHours);
    ...
    return {
        init: function init(settingsOverride) {
            var settings = {
                calendarUrl: 'https://www.googleapis.com/calendar/v3/calendars/4763d3r16hspfvtar25d8f0v9o@group.calendar.google.com/events?key=AIzaSyBJOz-oJyA3OXMZ-3gf0clgUROQMB7xTOE',
                past: true,
                upcoming: true,
                sameDayTimes: true,
                dayNames: true,
